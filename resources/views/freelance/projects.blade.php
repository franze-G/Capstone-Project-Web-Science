                    ...
                    @else
                        <div id="taskContainer" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-black">
                            @foreach ($filteredTasks as $task)
                                <div class="p-4 rounded-lg shadow-lg text-black font-sfpro bg-zinc-100 task-card"
                                    data-priority="{{ $task->priority }}" data-due-date="{{ $task->due_date }}">
                                    <h3 class="text-lg font-semibold mt-2 capitalize mb-2">{{ $task->title }}</h3>

                                    <p class="text-black">
                                        <span class="font-semibold">Due Date:</span>
                                        {{ \Carbon\Carbon::parse($task->due_date)->format('F j, Y \a\t g:i A') }}
                                    </p>
                                    <p class="">
                                        <span class="font-semibold">Assigned To:</span> {{ $task->assigned_firstname }}
                                        {{ $task->assigned_lastname }}
                                    </p>
                                    <p class="">
                                        <span class="font-semibold">Priority:</span> <span
                                            class="capitalize">{{ $task->priority }}</span>
                                    </p>
                            ...
                            due_date: "{{ \Carbon\Carbon::parse($task->due_date)->format('F j, Y \a\t g:i A') }}", priority: "{{ $task->priority }}" ,
