<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Lokalista</title>
    <!-- Fonts -->
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=figtree:400,600&display=swap" rel="stylesheet" />
    @vite('resources/css/app.css')
</head>

<body class="antialiased">
    <div
        class="flex flex-col min-h-screen px-4 dark:bg-black dark:text-white/50  selection:bg-olivegreen/60 selection:text-white">
        <header class="flex justify-between w-full max-w-6xl mx-auto py-4 px-4 mt-2 md:mt-5">
            <h1 class="flex font-tanpearl text-4xl md:text-6xl text-white">
                Lokal <span class="text-olivegreen">i</span> sta
            </h1>
            @if (Route::has('login'))
                <nav class=" lg:text-xl md:text-lg font-apercu md:flex">
                    @auth
                        <a href="{{ url('/dashboard') }}"
                            class="rounded-xl px-3 py-2 text-olivegreen ring-1 ring-transparent transition hover:text-emerald">
                            Dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="rounded-xl px-3 py-2 text-olivegreen hover:text-emerald ">
                            Login
                        </a>
                    @endauth
                </nav>
            @endif
        </header>
        <div class="flex flex-col items-center justify-center flex-grow mt-4 md:mt-10">
            <div class="flex flex-col justify-center items-center self-stretch text-white">
                <div class="flex flex-col justify-start items-center gap-7">
                    <div class="font-sans font-semibold text-white md:text-8xl text-4xl tracking-[0.035rem]">
                        Hiring made&nbsp;<span class="text-olivegreen">simple</span>
                        <p
                            class="text-white/80 self-stretch font-normal max-w-2xl mt-4 mx-auto text-center text-2xl leading-tight">
                            Lokalista gives you a platform to display your
                            job openings and a powerful dashboard to
                            manage your employee.
                        </p>
                    </div>

                    <div class="flex flex-col md:flex-row justify-center items-center gap-5 mt-10">
                        <img src="{{ asset('storage/images/model.svg') }}" alt="Model Image"
                            class="w-full md:mx-auto max-w-xs md:max-w-none">
                        <!-- text buttons -->
                        @guest
                            @if (Route::has('register'))
                                <div class="flex flex-col justify-center items-center gap-5 mt-10 md:mt-0">
                                    <p
                                        class="text-center text-lg md:text-2xl font-semibold tracking-[0.18px] font-sans m-1">
                                        Join as a client or a freelancer
                                    </p>
                                    <!-- Register buttons -->
                                    <div class="flex flex-col md:flex-row justify-center items-start gap-5">
                                        <!-- Freelancer Button -->
                                        @php
                                            $isFreelancer = request()->query('role') === 'freelancer';
                                            $isClient = request()->query('role') === 'client';
                                        @endphp
                                        <a href="{{ route('register', ['role' => 'freelancer']) }}"
                                            class="group flex flex-col justify-center items-center gap-2.5 w-56 h-32 p-6 bg-emerald/75 rounded-xl shadow-innershadow hover:bg-emerald {{ $isFreelancer ? 'border-2 border-emerald' : '' }}">
                                            <!-- Icons -->
                                            <div class="flex justify-between items-start self-stretch">
                                                <!-- Briefcase Icon -->
                                                <svg width="29" height="26" viewBox="0 0 29 26" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M18 17.4643C18 17.9576 17.6084 18.3571 17.125 18.3571H11.875C11.3916 18.3571 11 17.9576 11 17.4643V14.7857H0.5V22.8214C0.5 24.25 1.725 25.5 3.125 25.5H25.875C27.275 25.5 28.5 24.25 28.5 22.8214V14.7857H18V17.4643ZM25.875 5.85714H21.5V3.17857C21.5 1.75 20.275 0.5 18.875 0.5H10.125C8.725 0.5 7.5 1.75 7.5 3.17857V5.85714H3.125C1.725 5.85714 0.5 7.10714 0.5 8.53571V13H28.5V8.53571C28.5 7.10714 27.275 5.85714 25.875 5.85714ZM18 5.85714H11V4.07143H18V5.85714Z"
                                                        fill="#EBEBEB" fill-opacity="0.75" />
                                                </svg>
                                                <!-- Fill -->
                                                <div class="flex justify-center items-center">
                                                    <div
                                                        class="w-7 h-7 rounded-full border-4 border-gray-200/75 group-hover:bg-gray-200/80">
                                                    </div>
                                                </div>
                                            </div>
                                            <!-- Text -->
                                            <div
                                                class="text-white/80 text-lg font-semibold font-sfprodisplay tracking-tight">
                                                I'm a freelancer, looking for work
                                            </div>
                                        </a>
                                        <!-- Client Button -->
                                        <a href="{{ route('register', ['role' => 'client']) }}"
                                            class="group flex flex-col justify-center items-center gap-2.5 w-56 h-32 p-6 bg-emerald/75 rounded-xl shadow-innershadow hover:bg-emerald {{ $isClient ? 'border-2 border-emerald' : '' }}">
                                            <!-- Icons -->
                                            <div class="flex justify-between items-start self-stretch">
                                                <!-- Client Icon -->
                                                <svg width="31" height="26" viewBox="0 0 31 26" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M30.5 13.3906V12.6094C30.5 12.1777 30.1644 11.8281 29.75 11.8281H16.625V9.875H20C20.8283 9.875 21.5 9.17529 21.5 8.3125V2.0625C21.5 1.19971 20.8283 0.5 20 0.5H11C10.1717 0.5 9.5 1.19971 9.5 2.0625V8.3125C9.5 9.17529 10.1717 9.875 11 9.875H14.375V11.8281H1.25C0.835625 11.8281 0.5 12.1777 0.5 12.6094V13.3906C0.5 13.8223 0.835625 14.1719 1.25 14.1719H6.125V16.125H3.5C2.67172 16.125 2 16.8247 2 17.6875V23.9375C2 24.8003 2.67172 25.5 3.5 25.5H11C11.8283 25.5 12.5 24.8003 12.5 23.9375V17.6875C12.5 16.8247 11.8283 16.125 11 16.125H8.375V14.1719H22.625V16.125H20C19.1717 16.125 18.5 16.8247 18.5 17.6875V23.9375C18.5 24.8003 19.1717 25.5 20 25.5H29.75C30.1644 25.5 30.5 24.8003 30.5 23.9375V17.6875C30.5 16.8247 30.1644 16.125 29.75 16.125H27.125V14.1719H30.5C30.9144 14.1719 31.25 13.8223 31.25 13.3906H30.5Z"
                                                        fill="#EBEBEB" fill-opacity="0.75" />
                                                </svg>
                                                <!-- Fill -->
                                                <div class="flex justify-center items-center">
                                                    <div
                                                        class="w-7 h-7 rounded-full border-4 border-gray-200/75 group-hover:bg-gray-200/80">
                                                    </div>
                                                </div>
                                            </div>
                                            <!-- Text -->
                                            <div
                                                class="text-white/80 text-lg font-semibold font-sfprodisplay tracking-tight">
                                                I'm a client, managing a project
                                            </div>
                                        </a>
                                    </div>
                                </div>
                            @endauth
                        @endguest
                </div>
            </div>
        </div>
    </div>

    <footer class="py-16 text-center text-sm text-black dark:text-white/60 font-sfprorounded">
        Carl Andrei Del Rosario | Franzelighil Garcia
    </footer>
</div>
</body>

</html>
