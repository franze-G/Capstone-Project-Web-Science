<section>
    <div id="addTaskModal" class="fixed inset-0 items-center justify-center z-50 hidden">
        <div class="bg-white p-8 rounded-lg shadow-lg w-1/2">
            ...
            <form action="{{ route('addtask.post') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="datetime-local" id="due_date" name="due_date" class="mt-1 block w-full" required>
                </div>
                <div class="mb-4">
                    <label for="rate" class="block text-sm font-medium text-gray-700">Rate</label>
                    <input type="number" id="rate" name="rate" class="mt-1 block w-full" required>
                </div>
                <div class="mb-4">
                    <label for="image" class="block text-sm font-medium text-gray-700">Image</label>
                    <input type="file" id="image" name="image[]" multiple class="mt-1 block w-full" required>
                </div>
                <div class="flex justify-end">
                    <button type="button" id="closeModalButton" class="mr-4 px-4 py-2 bg-gray-300 rounded-lg">Cancel</button>
                    <button type="submit" id="addTaskModal" class="px-4 py-2 bg-black text-white rounded-lg">Add Task</button>
                </div>
            </form>
        </div>
    </div>
