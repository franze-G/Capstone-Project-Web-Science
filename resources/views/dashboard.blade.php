            ...
            <div class="bg-black overflow-hidden shadow-xl sm:rounded-xl">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-black text-black">
                        ...
                        <div class="bg-white p-4 rounded-lg shadow-md opacity-90">
                            <div class="flex items-center">
                                ...
                                <div class="ms-4">
                                    ...
                                    <p class="text-sm text-gray-300 mt-1">
                                        Pending:
                                        {{ $user->assignedProjects->where('status', 'pending')->where('created_by',
                                        auth()->user()->id)->count() }}
                                    </p>
                                    <p class="text-sm text-gray-300 mt-1">
                                        In-Progress:
                                        {{ $user->assignedProjects->where('status', 'in-progress')->where('created_by',
                                        auth()->user()->id)->count() }}
                                    </p>
                                    <p class="text-sm text-gray-300 mt-1">
                                        Completed:
                                        {{ $user->assignedProjects->where('status', 'completed')->where('created_by',
                                        auth()->user()->id)->count() }}
                                    </p>
                                    @endif
                                </div>
                            ...
                            <div class="mt-4 flex items-center">
                                ...
                                <button class="text-sm text-gray-400 underline" onclick="manageRole('{{ $user->id }}')">
                ...
                <div class="p-5 bg-slate-800">
                    <div class="flex justify-between items-center">
                        <div class="flex flex-col justify-center items-start gap-1 font-sfprodisplay tracking-tight">
