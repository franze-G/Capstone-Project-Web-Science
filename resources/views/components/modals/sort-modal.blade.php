    ...
    <div class="bg-black p-12 rounded-lg shadow-lg z-10">
      ...
      <form method="GET" action="{{ route('client.freelance-display') }}" class="flex flex-col">
        <select name="sort" class="p-2 bg-gray/30 rounded-lg focus:outline-none focus:ring-olivegreen">
          ...
          <option value="firstname_asc" {{ request('sort')=='firstname_asc' ? 'selected' : '' }}>First Name Ascending
          </option>
          <option value="firstname_desc" {{ request('sort')=='firstname_desc' ? 'selected' : '' }}>First Name Descending
          </option>
          <option value="position_asc" {{ request('sort')=='position_asc' ? 'selected' : '' }}>Position Ascending
          </option>
          <option value="position_desc" {{ request('sort')=='position_desc' ? 'selected' : '' }}>Position Descending
          </option>
        </select>
